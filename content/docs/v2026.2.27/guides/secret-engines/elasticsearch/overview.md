...
![Elasticsearch secret engine](/docs/v2026.2.27/images/guides/secret-engines/elasticsearch/elasticsearch_secret_engine_guide.svg)

You need to be familiar with the following CRDs:

- [AppBinding](/docs/v2026.2.27/concepts/vault-server-crds/auth-methods/appbinding)
- [SecretEngine](/docs/v2026.2.27/concepts/secret-engine-crds/secretengine)
- [ElasticsearchRole](/docs/v2026.2.27/concepts/secret-engine-crds/database-secret-engine/elasticsearch)

## Before you begin

- Install KubeVault operator in your cluster from [here](/docs/v2026.2.27/setup/README).
...
- [Deploy Vault Server](/docs/v2026.2.27/guides/vault-server/vault-server)

The KubeVault operator can manage policies and secret engines of Vault servers which are not provisioned by the KubeVault operator. You need to configure both the Vault server and the cluster so that the KubeVault operator can communicate with your Vault server.

- [Configure cluster and Vault server](/docs/v2026.2.27/guides/vault-server/external-vault-sever#configuration)

Now, we have the [AppBinding](/docs/v2026.2.27/concepts/vault-server-crds/auth-methods/appbinding) that contains connection and authentication information about the Vault server.
...
When a [SecretEngine](/docs/v2026.2.27/concepts/secret-engine-crds/secretengine) crd object is created, the KubeVault operator will enable a secret engine on specified path and configure the secret engine with given configurations.
...
By using [ElasticsearchRole](/docs/v2026.2.27/concepts/secret-engine-crds/database-secret-engine/elasticsearch), you can create a [role](https://www.vaultproject.io/docs/secrets/databases/elasticdb#setup) on the Vault server in Kubernetes native way.
...
> Don't have Vault CLI? Download and configure it as described [here](/docs/v2026.2.27/guides/vault-server/vault-server#enable-vault-cli)
