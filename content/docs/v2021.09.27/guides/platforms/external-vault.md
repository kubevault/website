...
- Manage Vault [policy](https://www.vaultproject.io/docs/concepts/policies.html) using [VaultPolicy](/docs/v2021.09.27/concepts/policy-crds/vaultpolicy) and [VaultPolicyBinding](/docs/v2021.09.27/concepts/policy-crds/vaultpolicybinding). Guides can be found [here](/docs/v2021.09.27/guides/policy-management/overview).
...
We have a Vault running which can be accessible by the address `http://vault.default.svc:8200` from Kubernetes cluster. KubeVault operator use [AppBinding](/docs/v2021.09.27/concepts/vault-server-crds/auth-methods/appbinding) to communicate with Vault. [AppBinding](/docs/v2021.09.27/concepts/vault-server-crds/auth-methods/appbinding) provides a way of specifying Vault connection information and credential. Following authentication methods are currently supported by KubeVault operator using AppBinding:
...
Vault authentication using AppBinding can be found in [here](/docs/v2021.09.27/concepts/vault-server-crds/auth-methods/overview).
...
If KubeVault operator uses the above AppBinding `vault-app`, then it will have the permission that is given to service account `policy-admin` by `policy-admin-role` role. Now, we are going to create [VaultPolicy](/docs/v2021.09.27/concepts/policy-crds/vaultpolicy) using `vault-app` AppBinding.
