...
![Vault Server](/docs/v2026.1.8-rc.0/images/guides/vault-server/vault_server_guide.svg)
...
Here we are using `inmem` backend which will lose data when Vault server pods are restarted. For production setup, use an appropriate backend. For more information about supported **backends** and **unsealer options** visit `VaultServer` CRD [documentation](/docs/v2026.1.8-rc.0/concepts/vault-server-crds/vaultserver)
