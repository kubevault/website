...
![Vault Policy CRD](/docs/v2026.5.18-rc.1/images/concepts/vault_policy.svg)
...
`spec.vaultRef` is a `required` field that specifies the name of an [AppBinding](/docs/v2026.5.18-rc.1/concepts/vault-server-crds/auth-methods/appbinding) reference which is used to connect with a Vault server. AppBinding must be on the same namespace with VaultPolicy object.
