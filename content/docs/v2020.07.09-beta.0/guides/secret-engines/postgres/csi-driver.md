...
- Install KubeVault CSI driver in your cluster from [here](/docs/v2020.07.09-beta.0/setup/csi-driver/install).
...
> Note: YAML files used in this tutorial stored in [examples](/docs/v2020.07.09-beta.0/examples/guides/secret-engines/postgres) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/docs)
...
Now, we have the [AppBinding](/docs/v2020.07.09-beta.0/concepts/vault-server-crds/auth-methods/appbinding) that contains connection and authentication information about the Vault server. And we also have the service account that the Vault server can authenticate.
...
Find how to update the policy for service account in [here](/docs/v2020.07.09-beta.0/guides/secret-engines/kv/csi-driver#update-vault-policy).
...
Let's say pod's service account name is `pod-sa` located in `trial` namespace. We need to create a [VaultPolicy](/docs/v2020.07.09-beta.0/concepts/policy-crds/vaultpolicy) and a [VaultPolicyBinding](/docs/v2020.07.09-beta.0/concepts/policy-crds/vaultpolicybinding) so that the pod has access to read secrets from the Vault server.
