...
When an `AzureAccessKeyRequest` is created, it makes a request to a Vault server for a new service principal under a `role`. Hence we need to deploy an [AzureRole](/docs/v2020.07.09-beta.0/concepts/secret-engine-crds/azure-secret-engine/azurerole) CRD before creating an `AzureAccessKeyRequest`.

![AzureAccessKeyRequest CRD](/docs/v2020.07.09-beta.0/images/concepts/azure_accesskey_request.svg)
...
AzureAccessKeyRequest `Spec` contains information about [AzureRole](/docs/v2020.07.09-beta.0/concepts/secret-engine-crds/azure-secret-engine/azurerole) and subjects.
...
`spec.roleRef` is a `required` field that specifies the [AzureRole](/docs/v2020.07.09-beta.0/concepts/secret-engine-crds/azure-secret-engine/azurerole) against which credentials will be issued.
