...
![AWSRole CRD](/docs/v2021.08.02/images/concepts/aws_role.svg)
...
`spec.vaultRef` is a `required` field that specifies the name of an [AppBinding](/docs/v2021.08.02/concepts/vault-server-crds/auth-methods/appbinding) reference which is used to connect with a Vault server. AppBinding must be in the same namespace with the AWSRole object.
