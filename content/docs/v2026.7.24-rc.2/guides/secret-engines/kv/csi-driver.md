...
![Secrets-store CSI architecture](/docs/v2026.7.24-rc.2/guides/secret-engines/csi_architecture.svg)
...
> Note: YAML files used in this tutorial stored in [examples](/docs/v2026.7.24-rc.2/examples/guides/secret-engines/kv) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
Now, we have the [AppBinding](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/auth-methods/appbinding) that contains connection and authentication information about the Vault server. And we also have the service account that the Vault server can authenticate.
...
Let's say pod's service account name is `pod-sa` located in `demo` namespace. We need to create a [VaultPolicy](/docs/v2026.7.24-rc.2/concepts/policy-crds/vaultpolicy) and a [VaultPolicyBinding](/docs/v2026.7.24-rc.2/concepts/policy-crds/vaultpolicybinding) so that the pod has access to read secrets from the Vault server.
