...
![VaultServer CRD](/docs/v2026.7.24-rc.2/images/concepts/vault_server.svg)
...
Specifies the name of the `VaultServerVersion` CRD. This CRD holds the image name and version of the Vault, Unsealer, and Exporter. To know more information about `VaultServerVersion` CRD see [here](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/vaultserverversion).
...
- [Filesystem](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/storage/filesystem)
- [Raft](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/storage/raft)

#### spec.unsealer

`spec.unsealer` is an optional field that specifies [Unsealer](https://github.com/kubevault/unsealer) configuration. Unsealer handles automatic initializing and unsealing of Vault. See [here](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/unsealer/overview) for Unsealer documentation.
...
`spec.relayPlacementRef` is an optional field that points to an [Open Cluster Management](https://open-cluster-management.io/) `Placement` object in the same namespace as the `VaultServer`. When set, the KubeVault operator deploys a [VaultRelay](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/vaultrelay) to every managed cluster selected by the Placement, using one `ManifestWork` per cluster. This turns the VaultServer into the hub of a hub-spoke deployment: databases in the selected spoke clusters can be managed through this Vault without being reachable from the hub.
...
`spec.isolateTenants` is an optional boolean field, the master opt-in for [tenant isolation](/docs/v2026.7.24-rc.2/guides/tenant-isolation/overview), defaulting to `false`. When enabled on an OpenBao (namespace-capable) distribution, a `SecretEngine` whose database lives in a client-org Kubernetes namespace is automatically provisioned in an OpenBao namespace keyed on the org's identity, and every dependent `{db}Role` / `SecretAccessRequest` inherits that namespace.
