...
1. The **spoke-side operator** resolves the org-id for each local client-org database (see [tenant isolation](/docs/v2026.7.24-rc.2/guides/tenant-isolation/overview)) and records the deduplicated, validated set it needs on the hub in one or more `NamespaceSlice` shards, under `spec.namespaces[]`.
...
- Learn about [Tenant Isolation with OpenBao Namespaces](/docs/v2026.7.24-rc.2/guides/tenant-isolation/overview), including the hub-spoke section describing how the hub learns which namespaces to create.
- Deploy the full hub-spoke model with OCM: [guide](/docs/v2026.7.24-rc.2/guides/hub-spoke/deploy-hub-spoke).
- Learn about the [VaultRelay](/docs/v2026.7.24-rc.2/concepts/vault-server-crds/vaultrelay) CRD.
