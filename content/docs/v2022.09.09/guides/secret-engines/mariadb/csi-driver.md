...
![Secrets-store CSI architecture](/docs/v2022.09.09/guides/secret-engines/csi_architecture.svg)
...
> Note: YAML files used in this tutorial stored in [examples](/docs/v2022.09.09/examples/guides/secret-engines/mariadb) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
Now, we have the [AppBinding](/docs/v2022.09.09/concepts/vault-server-crds/auth-methods/appbinding) that contains connection and authentication information about the Vault server. And we also have the service account that the Vault server can authenticate.
...
Let's say pod's service account name is `test-user-account` located in `demo` namespace. We need to create a [VaultPolicy](/docs/v2022.09.09/concepts/policy-crds/vaultpolicy) and a [VaultPolicyBinding](/docs/v2022.09.09/concepts/policy-crds/vaultpolicybinding) so that the pod has access to read secrets from the Vault server.
