...
![Secrets-store CSI architecture](/docs/v2022.01.11/guides/secret-engines/csi_architecture.svg)
...
- Install KubeVault operator in your cluster from [here](/docs/v2022.01.11/setup/README).
...
> Note: YAML files used in this tutorial stored in [examples](/docs/v2022.01.11/examples/guides/secret-engines/aws) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
- [Configure cluster and Vault server](/docs/v2022.01.11/guides/vault-server/external-vault-sever#configuration)

Now, we have the [AppBinding](/docs/v2022.01.11/concepts/vault-server-crds/auth-methods/appbinding) that contains connection and authentication information about the Vault server. And we also have the service account that the Vault server can authenticate.
...
Let's say pod's service account name is `test-user-account` located in `demo` namespace. We need to create a [VaultPolicy](/docs/v2022.01.11/concepts/policy-crds/vaultpolicy) and a [VaultPolicyBinding](/docs/v2022.01.11/concepts/policy-crds/vaultpolicybinding) so that the pod has access to read secrets from the Vault server.
