...
- Manage Vault [policy](https://www.vaultproject.io/docs/concepts/policies.html) using [VaultPolicy](/docs/0.2.0/concepts/policy-crds/vaultpolicy) and [VaultPolicyBinding](/docs/0.2.0/concepts/policy-crds/vaultpolicybinding). Guides can be found [here](/docs/0.2.0/guides/policy-management/overview).

- Manage [AWS secret engine](https://www.vaultproject.io/docs/secrets/aws/index.html#aws-secrets-engine) using [AWSRole](/docs/0.2.0/concepts/secret-engine-crds/awsrole) and [AWSAccessKeyRequest](/docs/0.2.0/concepts/secret-engine-crds/awsaccesskeyrequest). Guides can be found [here](/docs/0.2.0/guides/secret-engines/aws/overview).

- Manage [PostgreSQL Database secret engine](https://www.vaultproject.io/api/secret/databases/postgresql.html) using [PostgresRole](/docs/0.2.0/concepts/database-crds/postgresrole) and [DatabaseAccessRequest](/docs/0.2.0/concepts/database-crds/databaseaccessrequest). Guides can be found [here](/docs/0.2.0/guides/secret-engines/postgres/overview).

- Manage [MySQL Database secret engine](https://www.vaultproject.io/api/secret/databases/mysql-maria.html) using [MySQLRole](/docs/0.2.0/concepts/database-crds/mysql) and [DatabaseAccessRequest](/docs/0.2.0/concepts/database-crds/databaseaccessrequest). Guides can be found [here](/docs/0.2.0/guides/secret-engines/mysql/overview).

- Manage [MongoDB Database secret engine](https://www.vaultproject.io/api/secret/databases/mongodb.html) using [MongoDBRole](/docs/0.2.0/concepts/database-crds/mongodb) and [DatabaseAccessRequest](/docs/0.2.0/concepts/database-crds/databaseaccessrequest). Guides can be found [here](/docs/0.2.0/guides/secret-engines/mongodb/overview).

In this tutorial, we are going to show how we can use Vault operator for Vault which is not provisioned by Vault operator.

We have a Vault running which can be accessible by the address `http://vault.default.svc:8200` from Kubernetes cluster. Vault operator use [AppBinding](/docs/0.2.0/concepts/vault-server-crds/auth-methods/appbinding) to communicate with Vault. [AppBinding](/docs/0.2.0/concepts/vault-server-crds/auth-methods/appbinding) provides a way of specifying Vault connection information and credential. Following authentication methods are currently supported by Vault operator using AppBinding:
...
Vault authentication using AppBinding can be found in [here](/docs/0.2.0/concepts/vault-server-crds/auth-methods/overview).
...
If Vault operator uses the above AppBinding `vault-app`, then it will have the permission that is given to service account `policy-admin` by `policy-admin-role` role. Now, we are going to create [VaultPolicy](/docs/0.2.0/concepts/policy-crds/vaultpolicy) using `vault-app` AppBinding.
