...
In Vault operator, usually Vault connection information are handled by [AppBinding](/docs/0.2.0/concepts/vault-server-crds/auth-methods/appbinding). For [Userpass authentication](https://www.vaultproject.io/docs/auth/userpass.html), it has to be enabled and configured in Vault. To perform it,
