...
- The KubeVault operator installed on the hub **and** on every spoke cluster (the spoke operator reconciles the delivered `VaultRelay`). See the [setup guide](/docs/v2026.8.7/setup/README).
...
Request credentials the usual way with a `SecretAccessRequest`; see the [secret engine guides](/docs/v2026.8.7/guides/secret-engines/postgres/overview).
...
- [VaultRelay concept](/docs/v2026.8.7/concepts/vault-server-crds/vaultrelay)
- [VaultServer concept](/docs/v2026.8.7/concepts/vault-server-crds/vaultserver)
- [AppBinding concept](/docs/v2026.8.7/concepts/vault-server-crds/appbinding), in particular `spec.parameters.deploymentMode`
- [Secret engine guides](/docs/v2026.8.7/guides/secret-engines/postgres/overview)
- [Tenant Isolation with OpenBao Namespaces](/docs/v2026.8.7/guides/tenant-isolation/overview) — isolate each spoke's client-org databases into per-org OpenBao namespaces on the hub
