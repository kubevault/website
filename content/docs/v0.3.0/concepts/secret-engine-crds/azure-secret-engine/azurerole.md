...
![AzureRole CRD](/docs/v0.3.0/images/concepts/azure_role.svg)
...
`spec.vaultRef` is a `required` field that specifies the name of an [AppBinding](/docs/v0.3.0/concepts/vault-server-crds/auth-methods/appbinding) reference which is used to connect with a Vault server. AppBinding must be in the same namespace with the AzureRole object.
