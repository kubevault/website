...
> Note: YAML files used in this tutorial stored in [examples](/docs/v2022.09.05-rc.0/examples/guides/secret-engines/aws) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.aws-reader-role`.
...
$ kubectl apply -f docs/examples/guides/secret-engines/aws/policybinding.yaml
vaultpolicybinding.policy.kubevault.com/aws-reader-role created
```
