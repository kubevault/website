...
![Secrets-store CSI architecture](/docs/v2022.09.05-rc.0/guides/secret-engines/csi_architecture.svg)
...
- Install KubeVault operator in your cluster from [here](/docs/v2022.09.05-rc.0/setup/README).
...
> Note: YAML files used in this tutorial stored in [examples](/docs/v2022.09.05-rc.0/examples/guides/secret-engines/mariadb) folder in GitHub repository [KubeVault/docs](https://github.com/kubevault/kubevault)
...
- [Configure cluster and Vault server](/docs/v2022.09.05-rc.0/guides/vault-server/external-vault-sever#configuration)

Now, we have the [AppBinding](/docs/v2022.09.05-rc.0/concepts/vault-server-crds/auth-methods/appbinding) that contains connection and authentication information about the Vault server. And we also have the service account that the Vault server can authenticate.
...
Let's say pod's service account name is `test-user-account` located in `demo` namespace. We need to create a [VaultPolicy](/docs/v2022.09.05-rc.0/concepts/policy-crds/vaultpolicy) and a [VaultPolicyBinding](/docs/v2022.09.05-rc.0/concepts/policy-crds/vaultpolicybinding) so that the pod has access to read secrets from the Vault server.
...
When a VaultPolicyBinding object is created, the KubeVault operator create an auth role in the Vault server. The role name is generated by the following naming format: `k8s.(clusterName or -).namespace.name`. Here, it is `k8s.-.demo.mariadb-reader-role`.
...
spec:
  vaultRef:
    name: vault
  policyDocument: |
    path "your-database-path/creds/k8s.-.demo.mariadb-superuser-role" {
      capabilities = ["read"]
    }
---
apiVersion: policy.kubevault.com/v1alpha1
...
```bash
$ kubectl apply -f docs/examples/guides/secret-engines/mariadb/policy.yaml
vaultpolicy.policy.kubevault.com/mariadb-reader-policy created

$ kubectl apply -f docs/examples/guides/secret-engines/mariadb/policybinding.yaml
vaultpolicybinding.policy.kubevault.com/mariadb-reader-role created
```
...
NAME                                 STATUS    AGE
mariadb-reader-policy                  Success   8s

$ kubectl get vaultpolicybinding -n demo
NAME                                 STATUS    AGE
mariadb-reader-role                    Success   10s
```
